...
**Agent:** Claude (Paperclip implementer-claude, SIG-31) — addressed reviewer-codex's two SB-09A findings in `beta/PARITY.md`. Corrected the `endcase` row: the alpha does *not* clear every player's Harm as its confirm dialog promises — `c` is `P.ch`, the invoking client's own character, and `pushP()` writes only that client's own `players/{uid}` record, so with a GM who has no character nothing is cleared at all; the row now records the actual behaviour, the UI/implementation mismatch and the design decision that migrating it implies, and it is added as a seventh finding. Recorded the sponsor's resolved answer to the parity-bar question (`full-parity`, answered 2026-09-25) in place of the prose that still presented it as open, and fixed the stale "32 migrate rows" to the document's actual 41 so SB-13's gate is unambiguous. Verified the tally programmatically: 62 rows, 11 retain / 41 migrate / 10 defer. Documentation only.
**Branch:** `pc/SIG-31` — released for review

**Agent:** Claude (Paperclip implementer-claude, SIG-31) — added `beta/PARITY.md`, the SB-09A alpha feature-parity inventory: 62 capabilities read out of the actual code on `integration/beta` (not from handoff entries), each with a file/function citation, current beta coverage, one decision (11 retain, 41 migrate, 10 defer) and private-data/export notes. Raised one `ask_user_questions` on the cutover parity bar; the export/import rows are marked `NEEDS-SPONSOR` and their spec stays with SB-09C. Six findings recorded rather than fixed: face-down clue text is not actually private, puzzle answers ship to the player with the puzzle, the sealed page's UI copy overstates what `firebase.rules.json` enforces, the alpha cannot load a case file at all despite the documented loader patch, `dump-imp` is destructive and unvalidated, and `scripts/validate-setting.mjs` regex-parses `D100`/`BOOKS` out of `table/index.html`. Documentation only — no code, canon or protected file changed.
**Branch:** `pc/SIG-31` — released for review
