...
- At SB-02 verification time, `npm run smoke`, `npm run cases:validate`, `npm run html:sanity`, `npm run setting:validate`, and the then-current four reference tests passed. SB-03 later replaced that test module; run `node --experimental-strip-types --test beta/tests/model.test.mjs` for the current typed model. Alpha smoke reported no page errors.
...
Fable's independent re-review judged the Workers containment fix non-blocking by static inspection; it did not run the coordinator's checks. The live hosting topology was confirmed in Cloudflare's dashboard: `signal-bleed.com` is a static-assets Worker named `signal-bleed`, connected to `JohnWainee/signal-bleed`; it runs `npx wrangler deploy` from `/` with `main` as production. The active production version is the PR #6 main merge, while PR #7 branch builds have no traffic. Live main serves `/gm/` and `/table/` (200) and returns 404 for `/beta/gm/` and `/beta/src/app/main.ts`. Live main also still serves `/HANDOFF.md` (200); the reviewed `.assetsignore` returns 404 for it in local Wrangler and will close that exposure when deployed. The legacy Pages instructions are historical. SB-02's hosting gate is accepted. This fixture has no Firebase auth, persistence, real multiplayer, authorization, or private-network proof; the four reference tests do not establish those behaviors. Real iPad/phone/TV checks remain unperformed. The existing `/reference/gm/` redirect points back to `/gm/`, so SB-08 must create independent preservation before any eventual route replacement. No production asset-directory setting or protected file was changed by this work unit.
