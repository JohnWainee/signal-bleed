...
Status: M1 contract for SB-02–07; the isolated SB-04 emulator implements the backend portions, but Firebase rules/Functions and beta routes are not deployed. Supersedes the reference module's global revision and the README's earlier proposed path layout. Game canon remains in `rules/hawaii-setting-bible.md`. These engineering defaults do not approve new lifepath rules.
...
- Closed-beta creation is invitation-only: at most ten opaque, source-fixed room IDs exist for the entire cohort, each bound by trusted deployment configuration to one GM UID before creation. A URL or App Check token is not an invitation. Spent IDs are never recycled. More than ten rooms require a separate policy decision, not a routine configuration edit. The demo emulator may explicitly opt into open test slots; production fails closed if no binding is configured.
...
The SB-09B board is `{ schemaVersion: 1, revision, bleed, clues }`. Bleed is an integer 0–6. A clue is `{ schemaVersion: 1, id, revision, text, state }`, with at most 100 clues and text <=2000 characters. States are `staged`, `active`, `woven`, and `deep`; only active/woven clues are copied to `shared/board`. Staged/deep clues must be absent from non-GM payloads, not masked in the UI. Ordering is array order. Every create/update/move/clock command compares the GM board revision; accepted mutations increment it once and use the existing exact-command receipt rules. The public projection has its own revision and changes only when Bleed or the ordered active/woven clue projection changes, so private staging activity does not leak through public revision metadata. State changes archive rather than delete clues.
...
type Failure = 'UNAUTHENTICATED' | 'FORBIDDEN' | 'NOT_FOUND' | 'INVALID'
  ...
  | 'COMMAND_ID_REUSED' | 'ROOM_FULL' | 'ROOM_CLOSED' | 'DISCONNECTED';
...
6. Each command ID is unique per room and issuer. Successful result, canonical command payload and mutation are recorded atomically. Same ID + identical payload returns the original result without reapplying, even if the scene subsequently changed; same ID + different payload returns COMMAND_ID_REUSED. Authorization is rechecked before receipt lookup, so revocation never exposes old results. Keep accepted receipts for room lifetime in M1: there is no rolling eviction. Failed commands do not claim an ID. Room creation/admission commands follow the same idempotency guarantee. The caller generates and retains an opaque roomId before createRoom; retry uses that identical roomId and commandId. Creation atomically claims the unused assigned room and records its receipt. An existing room owned by another UID returns FORBIDDEN; a collision never transfers ownership. No room ID is newly generated inside a retry.
...
8. If several validation failures apply, authorization wins before receipt lookup; a reused ID is checked next, then whole-room closure, then command-specific missing entity, stale epoch, existing answer, closed prompt, and revision conflict. Return FORBIDDEN for unauthorized targets without revealing whether another player's entity exists.
9. Closed-beta capacity is enforced inside the trusted room transaction on the complete proposed room, including its receipt: at most 24 distinct applicant UIDs (including denied/revoked), 100 total prompt IDs (including expired/closed), 512 accepted receipts across issuers, and 512 KiB of UTF-8 serialized room JSON. Ordinary new commands stop at 511 receipts and 511 KiB, reserving one receipt and 1 KiB for `room.close`. A new command that would exceed a ceiling fails `ROOM_FULL` without mutation or receipt; an already accepted exact retry is still returned before capacity checks. These application limits are not a billing cap or exact RTDB wire-size measure. The GM can issue `room.close` once: it atomically records its receipt and makes the room read-only without erasing content. Exact accepted retries remain valid after closure; all other new mutations fail `ROOM_CLOSED`. GM-only status returns closed/capacity metadata without private room content. The client warns near a ceiling and on full/closed status. Manual export/deletion requires a separate owner-approved procedure; neither occurs automatically in M1.
