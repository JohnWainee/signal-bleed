...
- [ ] Screen reader — **deferred by sponsor:** enabling VoiceOver on the phone coincided with the active iPad room crashing. The session was stopped before reproduction or root-cause analysis, so no VoiceOver behavior passed and no causal claim is made. Revisit after the core beta basics are complete; then complete join/admission, scene update, private prompt/answer, disconnect/reconnect, revocation, and room closure, confirming useful announcements and removal of cleared private views.
...
A follow-up VoiceOver attempt was stopped after the operator reported that the iPad room crashed when VoiceOver was enabled on the phone. The failure was not reproduced or diagnosed, and accessibility validation is deferred rather than passed or waived.
